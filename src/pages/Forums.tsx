...
function ThreadRow({ thread }: { thread: ForumThread }) {
  ...
  return (
    <div
      ...
      className="group flex flex-col gap-1 px-3 py-2 hover:bg-white/[0.03] transition-colors rounded-md cursor-pointer"
    >
      {/* Row 1: full-width title + badges */}
      <div className="flex items-center gap-1.5 flex-wrap">
        <span className="text-sm text-white font-medium group-hover:text-vgd-orange transition-colors">
          {thread.title}
        </span>
        {hot && (
      ...
      {/* Row 2: author/age/category (left) + pills, counts, last reply (right) */}
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          {/* Reply / view counts */}
          <div className="flex items-center gap-2 text-[10px] text-vgd-muted whitespace-nowrap">
            <span><span className="text-white/80 font-medium">{formatCount(thread.reply_count)}</span> replies</span>
            <span><span className="text-white/80 font-medium">{formatCount(thread.view_count)}</span> views</span>
          </div>

          {/* Last reply */}
          {thread.last_reply_username && (
            <div className="hidden lg:flex items-center gap-1 text-[10px] text-vgd-muted whitespace-nowrap">
              <span>Last reply</span>
              <span className="text-white/70">{thread.last_reply_username}</span>
              <span>{formatAge(thread.last_reply_at || thread.last_active_at)}</span>
            </div>
          )}
        </div>
      </div>
