-- Update get_thread_page to return the real OP post (is_op=true) separately from replies
...
BEGIN
  ...
  FROM (
    ...
    WHERE thread_id = p_thread_id AND is_op = true
    LIMIT 1
  ) post_row
  LEFT JOIN LATERAL (
  ...
  -- Fetch paginated replies (is_op = false), ordered by created_at
  ...
  FROM (
    SELECT id, user_id, username, body, quoted_post_id, edited_at, created_at
    FROM public.forum_posts
    WHERE thread_id = p_thread_id AND is_op = false
