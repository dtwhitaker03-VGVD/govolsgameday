-- Lets the admin choose how long a manually-activated drive's pick window
-- stays open (30s / 1m / 2m from the new Live Drive Control panel), instead
-- of the previous hardcoded 30 seconds. p_window_seconds defaults to 30 so
-- every existing caller (live-cfbd-sync's automatic path included) is
-- unaffected.
...
BEGIN
  ...
  v_locked_at := NOW() + (GREATEST(1, COALESCE(p_window_seconds, 30)) || ' seconds')::INTERVAL;
